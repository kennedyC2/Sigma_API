...
test["Book_A_Test"] = (data, callback) => {
    ...
    switch (data.method) {
        ...
        case "post":
            ...
            if (firstname && lastname && total && paid && id && other && day && month && year && date && account && time && age && sex && religion && tribe && phone && email && dir && source && diagnosis && specimen && tokenID && selectedTest && result && companyID) {
                // Validate token
                        ...
                        file.read(dir + "/" + companyID + "/tests", "unsettled", (err, unsettled) => {
                            if (!err && unsettled) {
                                ...
                                if (unsettled[today] !== undefined) {
                                    unsettled[today] = { ...unsettled[today], [id]: data };
                                } else {
                                    unsettled[today] = {};
                                    unsettled[today] = { ...unsettled[today], [id]: data };
                                }
                                ...
                                file.update(dir + "/" + companyID + "/tests", "unsettled", unsettled, (err) => {
                                    if (!err) {
                                        // Update Stats
                                        update_stats(dir, companyID, data, (err, stats) => {
                                            if (!err && stats) {
                                                // Update Activity
