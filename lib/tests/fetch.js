            ...
            if (tokenID && companyID && type) {
                ...
                token.validate(tokenID, (err) => {
                    if (!err) {
                        ...
                        // Check Directory
                        folder.read(type + "/" + companyID + "/tests/unsettled/" + month, (err) => {
                            if (!err) {
                                // Try Unsettled Reading File
                                file.read(type + "/" + companyID + "/tests/unsettled/" + month, today, (err, details_1) => {
                                    if (!err && details_1) {
                                        // Try settled Reading File
                                        file.read(type + "/" + companyID + "/tests/settled/" + month, today, (err, details_2) => {
                                            if (!err && details_2) {
                                                // Return
                                                callback(200, {
                                                    unsettled: details_1,
                                                ...
                                                // Create settled File
                                                file.create(type + "/" + companyID + "/tests/settled/" + month, today, _data, (err) => {
                                                    if (!err) {
                                                        // Return
                                                        callback(200, {
                                                            unsettled: details_1,
                                        ...
                                        file.create(type + "/" + companyID + "/tests/unsettled/" + month, today, _data, (err) => {
                                            if (!err) {
                                                // Create settled File
                                                file.create(type + "/" + companyID + "/tests/settled/" + month, today, _data, (err) => {
                                                    if (!err) {
                                ...
                                folder.create(type + "/" + companyID + "/tests/unsettled/" + month, (err) => {
                                    if (!err) {
                                        // Create Settled Directory
                                        folder.create(type + "/" + companyID + "/tests/settled/" + month, (err) => {
                                            if (!err) {
                                                // Create File
                                                const _data = {};
