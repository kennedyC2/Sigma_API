...
// Component
const fetch_lab_activity = (data, callback) => {
    // Validate Method
    switch (data.method) {
        case "options":
            callback(200, {});
            ...
            if (tokenID && companyID && type) {
                ...
                token.validate(tokenID, (err) => {
                    if (!err) {
                        ...
                        // Check Directory
                        folder.read(type + "/" + companyID + "/lab_activity/" + month, (err) => {
                            if (!err) {
                                // Try Reading File
                                file.read(type + "/" + companyID + "/lab_activity/" + month, today, (err, details) => {
                                    if (!err && details) {
                                        // Return
                                        ...
                                        // Create File
                                        file.create(type + "/" + companyID + "/lab_activity/" + month, today, _data, (err) => {
                                            if (!err) {
                                                // Return
                                                callback(200, _data);
                                            } else {
                                ...
                                folder.create(type + "/" + companyID + "/lab_activity/" + month, (err) => {
                                    if (!err) {
                                        //  Define Data
                                        const _data = [];
...
// Export
module.exports = fetch_lab_activity;
