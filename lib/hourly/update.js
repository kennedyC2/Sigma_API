...
const update_hourly = (dir, ID, data, callback) => {
    ...
    if (payload && type && companyID) {
        ...
        const hour = helper.hour();

        // Check Folder
        folder.read(type + "/" + companyID + "/hourly/" + month, (err) => {
            if (!err) {
                // Try Reading File
                file.read(type + "/" + companyID + "/hourly/" + month, today, (err, details) => {
                    if (!err && details) {
                        // Update
                        ...
                        file.update(type + "/" + companyID + "/hourly/" + month, today, details, (err) => {
                            if (!err) {
                                callback(false);
                            } else {
                                callback(true);
                        ...
                        // Create File
                        file.create(type + "/" + companyID + "/hourly/" + month, today, _data, (err) => {
                            if (!err) {
                                // Return
                                callback(false);
                            } else {
                ...
                folder.create(type + "/" + companyID + "/hourly/" + month, (err) => {
                    if (!err) {
                        //  Define Data
                        const _data = {
                            amount: [0, 0, 0, 0, 0, 0, 0, 0],
