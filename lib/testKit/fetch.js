            ...
            if (tokenID && companyID && type) {
                ...
                token.validate(tokenID, (err) => {
                    if (!err) {
                        ...
                        // Check Directory
                        folder.read(type + "/" + companyID + "/testKits/" + month, (err) => {
                            if (!err) {
                                // Try Reading File
                                file.read(type + "/" + companyID + "/testKits/" + month, today, (err, details) => {
                                    if (!err && details) {
                                        // Return
                                        ...
                                        // Create File
                                        file.create(type + "/" + companyID + "/testKits/" + month, today, _data, (err) => {
                                            if (!err) {
                                                // Return
                                                callback(200, _data);
                                            } else {
                                ...
                                folder.create(type + "/" + companyID + "/testKits/" + month, (err) => {
                                    if (!err) {
                                        // Create File
                                        const _data = [];
                                        // Create File
