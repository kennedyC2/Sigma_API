                    ...
                    if (!err) {
                        ...
                        folder.read(dir + "/" + companyID + "/tests/" + month, (err) => {
                            if (!err) {
                                ...
                                file.read(type + "/" + companyID + "/tests/" + month, "unsettled", (err, details_1) => {
                                    if (!err && details_1) {
                                        // Try settled Reading File
                                        file.read(type + "/" + companyID + "/tests/" + month, "settled", (err, details_2) => {
                                            if (!err && details_2) {
                                                // Return
                                                callback(200, {
                                                    unsettled: details_1,
                                                ...
                                                // Create settled File
                                                file.create(type + "/" + companyID + "/tests/" + month, "settled", _data, (err) => {
                                                    if (!err) {
                                                        // Return
                                                        callback(200, {
                                                            unsettled: details_1,
                                        ...
                                        file.create(type + "/" + companyID + "/tests/" + month, "unsettled", _data, (err) => {
                                            if (!err) {
                                                // Create settled File
                                                file.create(type + "/" + companyID + "/tests/" + month, "=settled", _data, (err) => {
                                                    if (!err) {
                                ...
                                folder.create(type + "/" + companyID + "/tests/" + month, (err) => {
                                    if (!err) {
                                        // Create File
                                        const _data = {};
